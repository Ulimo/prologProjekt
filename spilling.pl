...
insertStore([H|T], [_|LT], ((Vertex, Start,End),Degree, Count), Index, RegCount, A, Output) :-
    Index =< Start,
    K is Index + 1,
...
insertStore([H|T], [LH|LT], ((Vertex, Start,End),Degree, Count), Index, RegCount, A, Output) :-
    ...
    LHLength =< RegCount,!,
    insertStore(T, LT, ((Vertex, Start,End),Degree, Count), Index,RegCount, [H|A], Output).
insertStore(List, _, ((Vertex, Start,End),Degree, Count), Index, _, A, Output) :-
    atom_concat(mem, Vertex, MemName),
    insertLoad(List, ((Vertex, Start,End),Degree, Count), Index, [['sw', Vertex, MemName]|A], Output).
    
    
insertLoad([], _, _, A, A).
insertLoad([H|T], ((Vertex, _,End),_, _), Index, A, Output) :-
    Index =< End,
    memberchk(Vertex, H),
    atom_concat(mem, Vertex, MemName),
...
findMinimumCost([_|T], Minimum, V, Output) :-
...
goToLine(((Vertex, Start,End),Degree), [_|T], Index, Count, Output) :-
    Index < Start,
    K is Index + 1,
...
calculateUsesVertex(((Vertex, Start,End),Degree), [_|T], Index, Count, Output) :-
    Index =< End,
    I is Index + 1,
